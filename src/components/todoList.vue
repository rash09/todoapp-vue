...
const priority_list = ["Optional", "Moderate", "Critical"];
    ...
    <div class="w-3/4 m-auto mt-30">
        <div class="flex border-b-2 border-black-300 p-3">
            ...
            <div class="grow-0 text-right"> 
                <button @click="sortItem">Sort &nbsp;<sortIcon />
                </button>&nbsp; &nbsp;
                <button @click="clearItem" class="text-center">Clear list &nbsp;<clearIcon/></button>
            </div>
        </div>


        <ul v-if="todoList?.length > 0">
            <li v-for="(todo, index) in todoList" :key="todo?.id" class="mb-3 mt-3"
                v-bind:class="[(index % 2=== 1)? 'even-item':'odd-item', priority_list[todo?.priority]]">
                <div>
                    {{ todo?.itemName }}
                    <span class="inline-block ml-20">{{ priority_list[todo?.priority] }}</span>
                    <span class="float-right">
                        <button @click="showEdit(todo?.id)"
                            class="inline-block text-xs p-2 text-custom-red border-2 border-custom-red mr-2"
                            data-modal-target="default-modal"
                            data-modal-toggle="default-modal">Edit</button>&nbsp;<button @click="deleteItem(todo?.id)"
                            class="text-xs bg-custom-red p-2 text-white mr-2">Delete</button>
                    </span>
                </div>
            </li>
        </ul>
