...
const updateTodo = (data) => {
    emit('updateTodos', { itemName: data.itemName, id: data.id, priority: data.priority});
}
</script>

<template>
    <div class="modal bg-gray-100 border-gray-600 pb-20" :class="showModal ?'block':'hidden'">
        <div class="relative text-right">
            <button id="closeButton" type="button"
                class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray-600 dark:hover:text-white"
                @click="closeItem"><clearIcon/> </button>
        </div>
        <div class="mt-5 ml-8">
            <form @submit.prevent="updateTodo(tempData)" v-if="listItems!==undefined && Object.keys(listItems).length>0">

                <input class="form-input px-2 py-2" type="text" v-model="tempData.itemName">
